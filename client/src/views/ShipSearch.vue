<template>
  <div style="width: 100%;">
      ...
      <div>
        ...
        <el-checkbox v-model="invoiceCheckAll"  @change="handleCheckAllChange3(invoiceCheckAll)"
                  >全选</el-checkbox>
        <el-checkbox-group v-model="formInline.invoiceItems" >
            <el-checkbox v-for="item in invoiceItems" :key="item" :label="item">
                {{item}}</el-checkbox>
        </el-checkbox-group>
        <el-checkbox v-model="emailCheckAll" @change="handleCheckAllChange4(emailCheckAll)"
                  >全选</el-checkbox>
...
<div class="common-layout" style="height: 100%;">
    ...
    <!--Excel表格导出-->
    <div>
        <el-table :data="tableData" style="width: 100%" sum-text="统计">
            <!-- 表格列定义 -->
            ...
            <el-table-column prop="equipmentModule" label="订单设备模块">
            ...
            <el-table-column prop="receivingDate" label="期望到货时间"></el-table-column>
            ...
            <el-table-column prop="emailDate" label="发货时间"></el-table-column>
            ...
            <el-table-column prop="signforDate" label="签收日期"></el-table-column>
            ...
            <el-table-column prop="acceptDate" label="验收日期"></el-table-column>
            <el-table-column prop="dealer" label="代理商信息"></el-table-column>
            <el-table-column prop="contractSignDate" label="合同签订日期"></el-table-column>
...
const onSubmit = () => {
    const val = { startDate: startDate.value, endDate: endDate.value ,keyword:keyword.value,formInline:formInline}
      axiosServer.AxiosPost(qs.stringify(val),'/ShipClient/GetShipSearch').then(res=>{
            ...
            item.orderDate = new Date(item.orderDate).toLocaleString('zh-CN', {
            hour12: false
            }),
            item.receivingDate = new Date(item.receivingDate).toLocaleString('zh-CN', {
            hour12: false
            });
            item.emailDate = new Date(item.emailDate).toLocaleString('zh-CN', {
            hour12: false
            });
            item.signforDate = new Date(item.signforDate).toLocaleString('zh-CN', {
            hour12: false
            }),
            item.acceptDate = new Date(item.acceptDate).toLocaleString('zh-CN', {
            hour12: false
            });
            item.contractSignDate = new Date(item.contractSignDate).toLocaleString('zh-CN', {
            hour12: false
            });
            item.paymentDate = new Date(item.paymentDate).toLocaleString('zh-CN', {
            hour12: false
            }),
            item.invoiceDate = new Date(item.invoiceDate).toLocaleString('zh-CN', {
            hour12: false
            });
            return item;
        });
